...
class HelpersTestCase(unittest.TestCase):
    ...
    def test_int2choice(self):
        assert 'True'   == int2choice(1)
        assert 'False'  == int2choice(2)
        assert 'Unsure' == int2choice(0)
        assert 'Unsure' == int2choice(-1)

    def test_choice2int(self):
        assert 1 == choice2int('True')
        assert 2 == choice2int('False')
        assert 0 == choice2int('Unsure')
        assert 0 == choice2int('')

    def test_appendage2choice(self):
        assert 'Concave or Flat' == appendage2choice(1)
        assert 'Convex' == appendage2choice(2)
        assert 'Unsure' == appendage2choice(0)
        assert 'Unsure' == appendage2choice(-1)

    def test_choice2appendage(self):
        assert 1 == choice2appendage('Concave')
        assert 1 == choice2appendage('Flat')
        assert 2 == choice2appendage('Convex')
        assert 0 == choice2appendage('Unsure')
        assert 0 == choice2appendage('')
